...
100 人覆盖：保留原始 QoE 计数作为比例来源，并先为开直播保留 2 人，其余 98 人按原始分布整数化。当前每个基础样本为短视频 36、长视频 27、会议/云游戏/语音/观看直播/游戏各 7、开直播 2。
...
当前仍固定媒体、G 和网络环境，调节有模型依据的媒体码率。套餐 60/25/15 与套餐→业务→位置→容忍度权重顺序为实验设置，尚非现网标定。套餐是最高权重维度，因此任意超级 VIP 单用户权重高于任意 VIP，任意 VIP 高于任意普通用户；这仍是总效用中的软优先，不是强制先达标。历史是模拟序列，不是实际执行反馈。
