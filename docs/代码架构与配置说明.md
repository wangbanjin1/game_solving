...
> 2026-09-20 更新：`User.app_id` 与 `applications` 规则表已纳入模型；`solver.group_start_iteration` 和 `initial_mos_band_width` 控制初态对齐后分组；`vip_guarantee` 支持 VIP App 级质差优先；动作及逐轮日志包含预测 KQI 和变化量。专项参数见 `configs/congestion_qoe_9_20_1.json`，详细语义见《拥塞实验设计与验证》。
...
全员基本保障可行时进入 `NORMAL`，候选必须满足基本保障。否则进入 `DEGRADED`，仍遵守全部硬条件，按套餐等级与业务优先级顺序优先减少基本保障不足人数、再减少缺口，最后改善效用。
...
| `game_solving/simulation/history.py` | 可选的受约束历史带宽模拟、MOS 记录及时间窗口聚合 |
| `game_solving/simulation/generator.py` | 生成编排、重试上限、失败报告与数据划分 |
| `game_solving/optimization/policy.py` | 硬条件、权重、效用、保障排序和动作评估 |
| `game_solving/optimization/history.py` | 按时间窗口和模型版本计算历史 MOS，供历史生成器或外部调用方使用 |
...
| `game_solving/evaluation/history.py` | 重读历史记录，复算 MOS、资源约束及用户历史均值 |
...
| `configs/congestion_exact.json` | 当前小规模精确参考实验 |
| `configs/congestion_qoe.json` | 当前仅 QoE 业务的百人拥塞实验 |
| `tests/fixtures/*.json` | 最小单元测试夹具，不是实验配置 |
...
配置覆盖规则：命令行优先于指定 JSON，指定 JSON 优先于默认 JSON。package_probs、business_probs、qoe_counts、minimum_qoe_counts、compliance_probs 与 extensions 按整体替换；其他对象递归合并，数组（含 joint_rows）整体替换。其他概率表调整时应写全各类别，或显式把不用的类别设为 0。JSON 不支持注释，字段说明集中在本节。修改公式、业务定义、码率量化或会话聚合会改变模型指纹，旧输入会被拒绝，必须重新生成。修改求解预算可直接复用输入。
...
`utility.switch_cost` 为固定 profile 版本的预留字段，目前不参与计算。`history_window_seconds` 供历史聚合使用；开启 `generation.history.enabled` 后生成器会模拟历史，详见[历史数据模拟说明](历史数据模拟说明.md)。`reference.max_nodes` 和 `generation.max_model_evaluations` 实际约束各自共享计数器，除节点/模型计算外也包含对应阶段的候选、比较、装配或历史生成操作。
...
| `policy.evaluation_packages` | `["vip"]` |
...
| `policy.package_order` | `["vip","normal"]` |
...
目前覆盖个人单业务、固定媒体、独立/联合人口、静态快照、可选合成历史和离线求解。合成历史用于初始化欠账补偿，尚未实现求解结果回写历史的多控制周期闭环。未实现团体关系、移动轨迹、真实网络测量噪声、在线执行、任意条件概率图及求解时分辨率/profile 切换。默认分布是用于算法验证的假设数据，后续可以独立替换采样器并用真实统计标定。参考枚举适合小规模；百人复杂候选会先触发工作量限制，返回已验证方案并保留真实停止原因。
