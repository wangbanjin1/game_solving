...
`history_window_seconds` 供历史聚合使用；开启 `generation.history.enabled` 后生成器会模拟历史，详见[历史数据模拟说明](历史数据模拟说明.md)。`reference.max_nodes` 和 `generation.max_model_evaluations` 实际约束各自共享计数器，除节点/模型计算外也包含对应阶段的候选、比较、装配或历史生成操作。
